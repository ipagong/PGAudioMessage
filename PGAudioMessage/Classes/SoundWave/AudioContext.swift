...
public final class AudioContext {
    ...
    public func render(targetSamples: Int = 100) -> [Float]? {
        ...
        guard let reader = try? AVAssetReader(asset: self.asset) else { return nil }
        ...
        guard reader.status == .completed || true else { return nil }
    ...
    private func processSamples(fromData sampleBuffer: inout Data,
        ...
        sampleBuffer.withUnsafeBytes { (body: UnsafeRawBufferPointer) in
            ...
            guard let samples = body.bindMemory(to: Int16.self).baseAddress else { return }
    ...
    private func getdB(from normalizedSamples: inout [Float]) {
        ...
        var ceil: Float = 0.0
        var noiseFloorMutable: Float = -70 // TODO: CHANGE THIS VALUE
