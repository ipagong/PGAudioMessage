...
extension AudioService {
    final public class Effector: NSObject {
        ...
        var result: AudioService.Effector.Result?
        
        public init(engine: AVAudioEngine = AVAudioEngine(),
                    mixerNode: AVAudioMixerNode = AVAudioMixerNode(),
                    effects: [AVAudioUnit],
...
extension AudioService.Effector {
    public func prepare() {
        ...
        guard let mixerNode = self.mixerNode else { return }
        ...
        self.effects.forEach{engine.attach($0)}
        
        let connects: [AVAudioNode] = [inputNode] + self.effects + [mixerNode]
    ...
    public func start(url: URL? = nil, options: AudioService.Option = AudioService.Effector.options) {
        ...
        guard let output = try? AVAudioFile(forWriting: url, settings: options.setting) else { return }
...
extension AudioService.Effector {
    public class Result {
        ...
        var validUrl: URL? {
            guard isFailed == true else { return nil }
            return self.url
        }
    }
}
